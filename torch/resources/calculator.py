...
class Calculator(object):
    ...
    def calculate(self):
        input_layer = Input(*self.inputs)
        modules = []
        for name, module in enumerate(self.net.acfe):
            modules.append(module)
        modules.append('permute')
        for name, module in enumerate(self.net.tdfe):
            modules.append(module)
        ...
        for module in modules:
            if module == 'permute':
                ...
                input_layer = self.Pool2d(module, input_layer, name)
                # exit()
            elif issubclass(type(module), torch.nn.Flatten):
                # print('Flatten')
                input_layer=self.Flatten(input_layer)
            elif issubclass(type(module), torch.nn.Linear):
                input_layer=self.Linear(module, input_layer)
                ...
                input_layer = self.Activation(module, input_layer, name)

    def Conv2d(self, module, layer_input):
        ...
        out_h = (layer_input.h - kh + 2 * ph) // sh + 1
        out_w = (layer_input.w - kw + 2 * pw) // sw + 1
        ...
        self.add_to_summary(*('Conv2d', (layer_input.c, layer_input.h, layer_input.w), out_shape, params, flops))
        return Input(*out_shape)

    def BatchNorm2d(self, module, layer_input):
        #params
        params = 0
        ...
        self.add_to_summary(*('BatchNorm2d', (layer_input.c, layer_input.h, layer_input.w), (layer_input.c, layer_input.h, layer_input.w), params, 0))
        # print(input.c, '-', input.h, '-', input.w)
        return layer_input

    def Pool2d(self, module, layer_input, name='Pool2d'):
        kh, kw = module.kernel_size
        sh, sw = module.stride
        ph, pw = module.padding, module.padding
        out_ch = layer_input.c
        scale = math.ceil if module.ceil_mode else math.floor
        out_h = scale((layer_input.h - kh + 2 * ph) / sh) + 1
        out_w = scale((layer_input.w - kw + 2 * pw) / sw) + 1
        flops = out_ch * out_h * out_w * kh * kw
        out_shape = (out_ch, out_h, out_w)
        self.add_to_summary(*(name, (layer_input.c, layer_input.h, layer_input.w), out_shape, 0, flops))
        # print(out_shape)
        return Input(*out_shape)

    def Permute(self, layer_input):
        # print(input.c, '-', input.h, '-', input.w)
        out_shape = (layer_input.h, layer_input.c, layer_input.w)
        self.add_to_summary(*("Permute", (layer_input.c, layer_input.h, layer_input.w), out_shape, 0, 0))
        # print(out_shape)
        return Input(*out_shape)

    def Flatten(self, layer_input):
        out_w = layer_input.c * layer_input.h * layer_input.w
        self.add_to_summary(*('Flatten', (layer_input.c, layer_input.h, layer_input.w), (1, out_w), 0, 0))
        # print((1,1,out_w))
        return Input(1,1,out_w)

    def Linear(self, module, layer_input):
        ...
        self.add_to_summary(*('Linear', (1, layer_input.w), (1, module.out_features), params, flops))
        # print((1,1,module.out_features))
        return Input(1,1,module.out_features)

    def Activation(self, module, layer_input, name):
        flops = layer_input.c * layer_input.h * layer_input.w
        in_shape = (1, layer_input.w) if layer_input.c == 1 and layer_input.h == 1 else (layer_input.c, layer_input.h, layer_input.w)
    ...
    def quick_summary(self):
        ...
        summary_str = "Input: {:.3f} MB, Params: {:,} ({:.3f} MB), Total: {:.2f} MB, FLOPs: {:,}".format(
